...
pub fn dedup_is_address_stable<S: ContentStore>(s: &S) {
...
pub fn isolation_between_stores<S: ContentStore>(a: &S, b: &S) {
...
pub fn address_is_pure_function_of_content<S: ContentStore>(s: &S) {
    let content = b"deterministic address";
    assert_eq!(s.put(content).unwrap(), ContentId::of(content));
}

pub fn address_is_stable_across_sizes<S: ContentStore>(s: &S) {
...
pub fn address_is_stable_across_reader_chunking<S: ContentStore>(s: &S) {
